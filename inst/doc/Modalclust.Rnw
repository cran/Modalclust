...
%\VignetteDepends{mvtnorm, zoo, multicore, class}
\usepackage{Sweave}
...
data. To partition the dataset into 4 partitions and to run
...
\noindent  
To partition the dataset into 4 partitions and to run
  them  in parallel in 4  processor one should use 
  
<<>>=
disc2d.phmac=phmac(disc2d,npart=4,parallel=T) #npart can be made equal to the number of processors the computer has.
@
